...
import { sessionOptions, AppSessionData } from '@/lib/session' // Adjusted path assuming lib is at src/lib
...
if (!process.env.OPENAI_API_KEY) {
  // This check is also inside ConversationManager, but good for early exit here too.
  console.error('CRITICAL: Missing OPENAI_API_KEY in chat route.')
  // Depending on how Next.js 13+ handles errors in route handlers,
  // you might want to return a NextResponse.error() or similar here
  // For now, an error will be thrown, halting startup if critical.
  throw new Error('Missing OPENAI_API_KEY environment variable in chat route')
}
...
async function getRecommendations(spendingData: SpendingData): Promise<any[]> {
  try {
    ...
    console.log("Calling CardGenius API with:", spendingData)
    ...
    if (cardResponse.ok) {
      ...
      console.log("CardGenius API success:", cardData)
