...
const debugLog = (...args: unknown[]) => {
  if (process.env.NODE_ENV === 'development' && process.env.NEXT_PUBLIC_CARDGENIUS_DEBUG === 'true') {
    console.debug(...args);
  }
};
...
export class ConversationManager {
  ...
  public updateSpendProfileField(
    ...
    debugLog(`Updated ${field} to ${value} in spendProfile`);
  }
  ...
  private mockOpenAIParseInitialIntent(userInput: string): InitialIntentParseResult {
    debugLog(`MOCK OpenAI Call: Parsing initial intent for: "${userInput}"`);
  ...
  private async _generateClarificationQuestion(term: string): Promise<string> {
    ...
    try {
      debugLog(`Generating clarification question for term: "${term}"`);
      ...
      if (question) {
        debugLog(`Generated clarification question: "${question}"`);
        return question;
      }
  ...
  private _processPendingBrands(): void {
    for (const brandObject of this.pendingBrandSpends) {
      ...
      if (categoryKey) {
        debugLog(`Mapping brand "${brandObject.brand_name}" to category "${categoryKey}" with amount: ${brandObject.amount}`);
  ...
  private async _getCardRecommendations(currentSpendProfile: SpendingData): Promise<CardGeniusResponse> {
    debugLog('Constructing payload for CardGenius API from current spend profile:', currentSpendProfile);
    ...
    debugLog('FLAT apiPayload being sent to fetchCardRecommendationsFromAPI:', apiPayload);
  ...
  private getNextQuestion(): keyof SpendingData['monthly'] | null {
    ...
    for (const key of spendProfileMonthlyKeys) {
      if (this.spendProfile.monthly[key] === null && !this.askedFields.has(key)) {
        debugLog(`[CM - getNextQuestion] Prioritizing question for '${key}' as its value is null and has not been successfully asked.`);
        return key;
      }
    }
    ...
    for (const fieldKeyFromOrder of questionAskingOrder) {
      if (!this.askedFields.has(fieldKeyFromOrder)) {
        ...
        debugLog(`[CM - getNextQuestion] Selecting '${fieldKeyFromOrder}' from questionAskingOrder as it's not in askedFields.`);
        return fieldKeyFromOrder;
      }
    }

    debugLog(`[CM - getNextQuestion] No suitable next question found. All known null-value fields have been addressed or are in askedFields.`);
    return null;
  }

  public async determineNextAction(): Promise<NextAction> {
    debugLog("[CM - determineNextAction] Entered determineNextAction.");
    try {
      debugLog("[CM - determineNextAction] Calling _processPendingBrands.");
      this._processPendingBrands();
      debugLog("[CM - determineNextAction] Finished _processPendingBrands.");
    } catch (e: any) {
    ...
    if (this.ambiguousTerms.length > 0) {
      ...
      try {
        debugLog(`[CM - determineNextAction] Found ambiguous term: "${termToClarify}". Generating clarification question.`);
        ...
        debugLog(`[CM - determineNextAction] Clarification question for "${termToClarify}": "${clarificationQuestion}"`);
        return {
    ...
    try {
      debugLog("[CM - determineNextAction] Calling hasEnoughDataForRecommendations.");
      enoughData = this.hasEnoughDataForRecommendations();
      debugLog(`[CM - determineNextAction] hasEnoughDataForRecommendations returned: ${enoughData}`);
    } catch (e: any) {
    ...
    if (enoughData) {
      try {
        debugLog("[CM - determineNextAction] Calling _getCardRecommendations as enoughData is true.");
        const recommendations = await this._getCardRecommendations(this.spendProfile);
        debugLog("[CM - determineNextAction] _getCardRecommendations returned. Preparing CALL_API action.");
        return {
          type: 'CALL_API',
    ...
    try {
      debugLog("[CM - determineNextAction] Calling getNextQuestion.");
      nextField = this.getNextQuestion();
      debugLog(`[CM - determineNextAction] getNextQuestion returned: ${nextField}`);
    } catch (e: any) {
    ...
    if (!nextField) {
      ...
      debugLog("[CM - determineNextAction] No nextField returned, but not enough data for recommendations. Ending conversation.");
      return {
  ...
  public async parseInitialIntent(userInput: string): Promise<NextAction> {
    ...
    try {
      debugLog("Attempting to parse initial intent for:", userInput, "with OpenAI API.");
      ...
      debugLog("Parsed OpenAI response:", parsedResult);

      if (parsedResult.unrelated_query) {
        debugLog("User query seems unrelated.");
...
Do not include any other text or explanations outside this JSON object.`;
    ...
    try {
      debugLog(`Processing user answer for ${fieldKey}: "${userAnswer}"`);
      const response = await this.openai.chat.completions.create({
      ...
      debugLog(`Raw OpenAI JSON for ${fieldKey}:`, resultJsonString);
      ...
      debugLog(`Parsed OpenAI response for ${fieldKey}:`, parsedResult);
      ...
      debugLog(`[CM - processUserResponse] Successfully processed and marked ${fieldKey} as asked.`);

      debugLog(`[CM - processUserResponse] About to call determineNextAction after processing ${fieldKey}.`);
  ...
  public async processClarificationResponse(
    ...
    debugLog(`Processing user answer: "${userAnswer}" for clarified term: "${termClarified}" (question was: "${clarificationQuestionAsked}")`);
    ...
    try {
      ...
      debugLog(`Raw OpenAI JSON for clarification of ${termClarified}:`, resultJsonString);
      const parsedResult: ClarificationParseResult = JSON.parse(resultJsonString);
      debugLog(`Parsed OpenAI response for clarification of ${termClarified}:`, parsedResult);
  ...
  private hasEnoughDataForRecommendations(): boolean {
    ...
    debugLog(`[CM - hasEnoughDataForRecommendations] Conditions: hasAtLeastOnePositiveSpend=${hasAtLeastOnePositiveSpend}, hasIdentifiedButUnaskedSpends=${hasIdentifiedButUnaskedSpends}, noPendingBrands=${noPendingBrands}, noAmbiguousTerms=${noAmbiguousTerms}`);

    if (hasIdentifiedButUnaskedSpends) {
      debugLog("[CM - hasEnoughDataForRecommendations] Returning false: Found categories identified (value is null) but their amounts haven't been asked/provided yet.");
      return false; 
    }
